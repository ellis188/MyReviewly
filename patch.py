...
# REMOVE any existing hamburger injection first
old_count = js.count("<style>.hamburger")
if old_count > 0:
    print(f"Found {old_count} existing hamburger injection(s) - removing...")
    # Remove everything from <style>.hamburger to the closing </script> before </body>
    while "<style>.hamburger" in js:
        start = js.find("<style>.hamburger")
        # Find the </script> that closes this injection block
        end = js.find("</script>", start)
        if end > 0:
            end += len("</script>")
            removed = js[start:end]
            js = js[:start] + js[end:]
            print(f"  Removed {len(removed)} bytes")
        else:
            break
    print(f"After removal: {len(js)} bytes")

# Now inject the fresh hamburger with How it works
...
for bp in body_patterns:
    if bp in js:
        ...
        print(f"OK: fresh hamburger injected")
        inserted = True
        break
