...
# Remove old hamburger injections
...
# 1. Change "Send Myself a Demo" buttons to "Start Free Trial"
for old, new in [
    ...
    if count > 0:
        ...
        print(f"Replaced '{old}' ({count}x)")

# 2. Fix the subtext: "No card required · First 25 customers free" 
# Change to two lines with "No credit card required" below
for old, new in [
...
for old, new in gbp_replacements:
    if old in js:
        js = js.replace(old, new)
        print(f"GBP: replaced '{old[:40]}'")
